...
function Scene({
  ...
  return (
    <>
      <Blob theme={theme} complexity={complexity} speed={speed} interactive={interactive} />
      {showParticles && particleCount > 0 && (
        <Particles count={particleCount} color={particleColors[theme]} />
      )}
      <ambientLight intensity={showEnvironment ? 0.3 : 0.42} />
      <pointLight position={[10, 10, 10]} intensity={showEnvironment ? 1.5 : 1.15} color="#ffffff" /> 
      <pointLight position={[-10, -10, -10]} intensity={showEnvironment ? 1.0 : 0.75} color="#4444ff" />
      {showEnvironment && (
